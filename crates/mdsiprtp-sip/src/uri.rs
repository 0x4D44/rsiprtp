...
impl SipUri {
    ...
    pub fn parse(s: &str) -> Result<Self, SipError> {
        ...
        let (scheme, rest) = if let Some(idx) = s.find(':') {
            ...
            if scheme != "sip" && scheme != "sips" {
                return Err(SipError::Parse(format!("Invalid SIP URI scheme: {}", scheme)));
            }
            (scheme, &s[idx + 1..])
        } else {
        ...
        let (user, host_port) = if let Some(idx) = user_host_port.find('@') {
            (Some(user_host_port[..idx].to_string()), &user_host_port[idx + 1..])
        } else {
            (None, user_host_port)
        };
        ...
        let (host, port) = if host_port.starts_with('[') {
            ...
            if let Some(bracket_end) = host_port.find(']') {
                ...
                return Err(SipError::Parse("Invalid IPv6 address in SIP URI".to_string()));
            }
    ...
    pub fn effective_port(&self) -> u16 {
        self.port.unwrap_or(if self.is_secure() { 5061 } else { 5060 })
    }
    ...
    pub fn has_param(&self, name: &str) -> bool {
        let name_lower = name.to_lowercase();
        self.params.iter().any(|(k, _)| k.to_lowercase() == name_lower)
    }
...
impl SipUriBuilder {
    ...
    pub fn build(self) -> Result<SipUri, SipError> {
        let host = self.host.ok_or_else(|| SipError::InvalidHeader("Missing host in SIP URI".to_string()))?;
