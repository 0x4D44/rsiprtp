...
fn parse_key_params(s: &str) -> Result<ParsedKeyParams, String> {
    ...
    for part in parts.iter().skip(1) {
        if let Some(exp_str) = part.strip_prefix("2^") {
            ...
            let exp: u32 = exp_str
                .parse()
                .map_err(|_| "Invalid lifetime exponent")?;
            lifetime = Some(1u64 << exp);
        } else if part.contains(':') {
            // MKI
            let mki_parts: Vec<&str> = part.split(':').collect();
            if mki_parts.len() == 2 {
                let value: u32 = mki_parts[0]
                    .parse()
                    .map_err(|_| "Invalid MKI value")?;
                let len: u8 = mki_parts[1]
                    .parse()
                    .map_err(|_| "Invalid MKI length")?;
                mki = Some((value, len));
            }
        }
    }
