    ...
    pub fn handle_timeout(&mut self, timer: Timer) {
        match (self.state, timer) {
            (State::Completed, Timer::G) => {
                ...
                self.actions.push(Action::SetTimer(Timer::G, self.retransmit_interval));
            }
        ...
        match self.state {
            State::Proceeding => {
                if (100..200).contains(&code) {
                    ...
                    self.actions.push(Action::SetTimer(Timer::H, self.timers.timer_h()));
                }
            }
            _ => {
...
mod tests {
    ...
    fn test_failure_response() {
        ...
        let actions = tx.poll_actions();
        assert!(actions.iter().any(|a| matches!(a, Action::SetTimer(Timer::G, _))));
        assert!(actions.iter().any(|a| matches!(a, Action::SetTimer(Timer::H, _))));
    }
    ...
    fn test_ack_received() {
        ...
        let actions = tx.poll_actions();
        assert!(actions.iter().any(|a| matches!(a, Action::Event(Event::AckReceived))));
    }
